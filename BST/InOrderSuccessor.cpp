...
Node * InOrderSuccessor(Node *p,Node *s)
{
    Node * ans=NULL;
    if(p->right!=NULL)
    {
...
int main()
{
   ...
   // cout.tie(NULL);
    ...
    int val;
    cout<<"Enter a value for finding Inorder Successor: ";
    cin>>val;
    Node *ptr=Search(root,val);
    Node *p=InOrderSuccessor(ptr,root);

    if(p==NULL)
    {
        cout<<"No Successor for given value!!!"<<endl;
    }
    else
    {
        cout<<p->data<<endl;
